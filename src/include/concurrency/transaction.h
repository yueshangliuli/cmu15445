 ...
 *2PL是一种并发控制协议，用于确保事务的隔离性和一致性。它包含两个主要阶段：增长阶段（GROWING）和收缩阶段（SHRINKING）。
 *
...
class Transaction {
 public:
  ...
  auto Judge(const table_oid_t &oid) -> bool {
    int a = (*s_row_lock_set_)[oid].size();
    int b = (*x_row_lock_set_)[oid].size();
    return a + b;
  }
  auto Insert(const table_oid_t &oid, int s) -> void {
    switch (s) {
      case 0:
        s_table_lock_set_->insert(oid);
        break;
      case 1:
        x_table_lock_set_->insert(oid);
        break;
      case 2:
        is_table_lock_set_->insert(oid);
        break;
      case 3:
        ix_table_lock_set_->insert(oid);
        break;
      case 4:
        six_table_lock_set_->insert(oid);
        break;
    }
  }
  auto Delete(const table_oid_t &oid, int s) -> void {
    switch (s) {
      case 0:
        s_table_lock_set_->erase(oid);
        break;
      case 1:
        x_table_lock_set_->erase(oid);
        break;
      case 2:
        is_table_lock_set_->erase(oid);
        break;
      case 3:
        ix_table_lock_set_->erase(oid);
        break;
      case 4:
        six_table_lock_set_->erase(oid);
        break;
    }
  }
  auto Insertt(const table_oid_t &oid, int s, RID rid) -> void {
    switch (s) {
      case 0:
  ...
  auto Deletee(const table_oid_t &oid, int s, RID rid) -> void {
    switch (s) {
      case 0:
        (*s_row_lock_set_)[oid].erase(rid);
        // std::cout << "delete" << std::endl;
        break;
      case 1:
        (*x_row_lock_set_)[oid].erase(rid);
        break;
    }
  }
