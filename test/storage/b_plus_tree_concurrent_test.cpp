...
TEST(BPlusTreeConcurrentTest, InsertTest1) {
  ...
  LaunchParallelTest(6, InsertHelper, &tree, keys);
...
TEST(BPlusTreeConcurrentTest, MixTest2) {
  ...
  for (; i < int64_t(dynamic_keys.size()); i++) {
    /* if (dynamic_keys[i] == 51) {
       break;
     }*/
    index_key.SetFromInteger(dynamic_keys[i]);
    tree.Remove(index_key, transaction);
  }
  for (; i < int64_t(dynamic_keys.size()); i++) {
    /*if(dynamic_keys[i] == 2825){
      break;
    }*/
    index_key.SetFromInteger(dynamic_keys[i]);
    tree.Remove(index_key, transaction);
  ...
  // GenericKey<8> index_key;
  // index_key.SetFromInteger(99940);
...
TEST(BPlusTreeConcurrentTest, SequentialMixTest) {
  ...
  // create and fetch header_page
  // first, populate index
  std::vector<int64_t> keys = {2, 1, 4, 3, 6, 5, 8, 7, 10, 9};
  int64_t scale_factor = 1000;
  for (int64_t key = 1; key < scale_factor; key++) {
    for (int i = 0; i < 10; i++) {
